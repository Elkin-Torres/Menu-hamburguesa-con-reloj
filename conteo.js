...
export function conteo(fechaFutura) {
  ...
  progreso = setInterval(() => {
    ...
    let minutosRestantes = ("0" + Math.floor((fechaConteo / 60) % 60)).slice(-2);
    let horasRestantes = ("0" + Math.floor((fechaConteo / 3600) % 24)).slice(-2);
    ...
    if(fechaConteo <= 1 ){añoNuevo.insertAdjacentText("afterbegin", "¡Feliz año!")};
